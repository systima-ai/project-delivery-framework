...
- **`pdf-map-stakeholders` (Marcus / Mobilization).** New stakeholder analysis layer that turns the flat `STAKEHOLDERS.md` register into an engagement instrument. Produces `00-constitution/STAKEHOLDER-MAP.md` (living document): an **influence/interest grid** (Manage Closely / Keep Satisfied / Keep Informed / Monitor), a **salience** read per stakeholder (power / legitimacy / urgency, deriving a priority tier rather than eliciting one), and a **management profile** for each material stakeholder (motivations, the metrics they are judged on, working/communication style as free-text, trust level, and the one thing to get right). The map cites the register by ID and never restates its facts, keeping a single writer per fact; its per-quadrant cadences feed `pdf-create-comms-plan`. Includes `references/stakeholder-analysis-guide.md`. Models are public scholarship (Mendelow; Eden & Ackermann; Mitchell, Agle & Wood; Cockburn; the trust equation) and cited as such; vendor-neutral throughout.
- **`pdf-relationship-health` (Helena / Governance).** New client-relationship health card, same dual-view anatomy as the quality health cards, RAG-rated across six sub-dimensions adapted from the trust equation: **Credibility, Reliability, Candour, Client-Centred Orientation, Engagement Cadence, Alignment**. Designed as a **leading indicator**: a relationship usually sours before delivery metrics move, and this surfaces the early behavioural signals while they are cheap to fix. Evidence must be behavioural, not a feeling; overall = worst sub-dimension unless justified; 28-day default cadence. RED on Candour or Client-Centred Orientation routes to a 1:1 (Iris); overall RED routes to an escalation memo or stakeholder reset (Helena). Output: `05-governance/relationship-health-cards/<date>.md`. Includes `references/relationship-evidence-guide.md`.
- **`pdf-diagnose-situation` (meta utility).** New standalone diagnostic (sibling of `pdf-elicit` / `pdf-red-team`) for when a live engagement goes sideways. Matches the situation to one of six generic delivery patterns (ambiguous opportunity; health drifting green→amber; expectation gap surfaced late amber→red; over-specified/hard-to-source role; out-of-contract request/scope creep; general stakeholder dissatisfaction), then walks **Clarify → Frame → Options → Recommend → Plan → Communicate** and **routes each action to the workflow that executes it** (change request, escalation decision, relationship-health, capacity plan, 1:1). Frames the problem in the stakeholder's terms before generating options; refuses single-option diagnostics and recommendations with no named trade-off. Output: `diagnostics/<date>-<slug>.md`. Includes `references/situation-patterns.md`. Method is generic structured problem-solving (A3/PDCA lineage); vendor-neutral.

### Changed

- **`pdf-create-change-request` (Klaus).** Adds a **cumulative-creep check**: before composing, the workflow scans prior CRs and, when three or more unconsolidated `minor` changes (including light-path "delighters") have accumulated, sets `cumulative_creep_watch: true` and adds a consolidation note to the recommendation. Many small uncontracted changes can sum to a material impact; the trend is now made visible rather than absorbed silently. Approver routing also consults `STAKEHOLDER-MAP.md` so a Manage-Closely / Keep-Satisfied approver is named explicitly. Validation and the inline red-team updated accordingly.
- **`pdf-create-kickoff-deck` (Marcus).** Adds a canonical **"How we work together"** slide (now 15 slides): a bidirectional ways-of-working alignment covering feedback and escalation norms, decision protocols, working-hours/time-zone overlap, onboarding/rotation handling, and role mapping. It states how we work and asks the client to state how they work, so misaligned expectations surface in week one rather than at the first escalation. Validation updated.
- **`STAKEHOLDERS.md` vs `STAKEHOLDER-MAP.md` separation** documented in `pdf-create-charter/references/charter-fields-guide.md`: the register holds facts, the map holds the analysis, and the map cites the register by ID.
- Marcus's and Helena's menus/scans, `pdf-help.csv` (three new rows), `ARCHITECTURE.md` (§3 influences), and the README counts updated for the new skills.
